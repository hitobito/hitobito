...
# app/javascript/images *above* every wagon's app/assets/images, since only
# in the vendor/wagons production layout do wagon paths live under
# Rails.root. We want the opposite: a wagon's file should win over a core
# file of the same name (e.g. favicon.ico, logo.png) in every layout - mirrors
# the previous wagon-media/media priority in
...
Rails.application.config.after_initialize do |app|
  wagon_paths, other_paths = app.config.assets.paths.partition { |path| wagon_image_paths.include?(path.to_s) }
