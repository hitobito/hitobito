...
describe Event::PreconditionChecker do
  ...
  describe "defaults" do
    its(:course_minimum_age) { is_expected.to be_blank }
  ...
  describe "minimum age person" do
    ...
    context "has no birthday" do
      ...
      its("errors_text.last") { is_expected.to eq too_young_error }
    end
  ...
  describe "qualification" do
    ...
    context "person without 'super lead'" do
      its(:valid?) { is_expected.to be_falsey }
      its("errors_text.last") { is_expected.to =~ /Qualifikationen fehlen: Super Lead/ }
    end
    ...
    context "person with valid 'super lead'" do
      ...
      its(:valid?) { is_expected.to be_truthy }
      its(:errors_text) { is_expected.to == [] }
    end
    ...
    context "multiple preconditions" do
      ...
      its("errors_text.last") { is_expected.to =~ /Qualifikationen fehlen: Super Lead, Group Lead/ }

      context "missing only one" do
        before { qualifications << Fabricate(:qualification, qualification_kind: sl, start_at: valid_date) }

        its(:valid?) { is_expected.to be_falsey }
        its("errors_text.last") { is_expected.to =~ /Qualifikationen fehlen: Group Lead/ }
      end
      ...
      context "in multiple groups" do
        ...
        its("errors_text.last") { is_expected.to =~ /Erforderliche Qualifikationen fehlen/ }
