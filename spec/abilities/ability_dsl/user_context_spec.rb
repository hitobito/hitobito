...
describe AbilityDsl::UserContext do
  ...
  context :top_leader do
    ...
    its(:all_permissions) {
      is_expected.to contain_exactly(:admin, :finance, :impersonation, :layer_and_below_full, :layer_and_below_read,
        :contact_data)
    }
  ...
  context :multiple_roles do
    let(:user) do
      ...
      Fabricate(Group::TopGroup::Leader.sti_name.to_sym, group: groups(:top_group), person: p, start_on: 3.years.ago,
        end_on: 2.years.ago)
      p
    end
    ...
    its(:admin) { should be_falsey }
    its(:all_permissions) {
      is_expected.to contain_exactly(:layer_and_below_full, :layer_and_below_read, :group_read, :group_and_below_read,
  ...
  describe "permission implication for groups" do
    ...
    describe "#all_permissions" do
      it "includes implicit permissions for groups" do
        ...
        expect(subject.all_permissions).to contain_exactly(:finance, :layer_and_below_read, :invoices_full,
          :invoices_read, :admin)
      end
      ...
      it "includes implicit permissions expanded with implications" do
        ...
        expect(subject.all_permissions).to contain_exactly(:finance, :layer_and_below_read, :group_and_below_full,
    ...
    describe "#permission_group_ids(:permission)" do
      it "includes implicit permissions for groups" do
        ...
        # see test setup
        expect(subject.permission_group_ids(:invoices_full)).to match_array([groups(:bottom_group_one_one).id,
          groups(:bottom_group_one_one_one).id, groups(:bottom_group_one_two).id])
