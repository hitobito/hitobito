...
describe Wallets::GoogleWallet::Client do
  ...
  before do
    ...
    # Reset class-level token cache so each test starts without a cached token
    described_class.instance_variable_set(:@token, nil)
    described_class.instance_variable_set(:@token_expires_at, nil)
  ...
  describe "#generate_save_url" do
    ...
    it "falls back to client_email as iss when issuer_email is nil" do
      allow(Wallets::GoogleWallet::Config).to receive(:issuer_email).and_return(nil)
      jwt_token = client.generate_save_url("issuer.object_id").split("/").last
      claims = JWT.decode(jwt_token, rsa_key.public_key, true, algorithm: "RS256").first
      expect(claims["iss"]).to eq("sa@project.iam.gserviceaccount.com")
    end
  end
  ...
  describe "token management" do
    it "renews expired token" do
      ...
      # Expire the class-level token
      described_class.instance_variable_set(:@token_expires_at, 1.minute.ago)
      ...
      client.get_object("test_id")
      expect(new_authorizer).to have_received(:fetch_access_token!)
    end
  end
  ...
  describe "error handling" do
    ...
    it "includes details in the error message when present" do
      error_body = {
        error: {
      ...
      expect {
        client.create_or_update_object({id: "bad"})
      }.to raise_error(RestClient::BadRequest, /Invalid field.*details:/)
    end
  end
end
