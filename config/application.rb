...
module Hitobito
  ...
  class Application < Rails::Application
    ...
    if ENV["RAILS_CACHE_REDIS_URL"].present?
      additional_redis_options = YAML.load("{ #{ENV['RAILS_CACHE_REDIS_CONFIG']} }").symbolize_keys
      config.cache_store = :redis_cache_store, { url: ENV["RAILS_CACHE_REDIS_URL"] }.merge(additional_redis_options)
    else
