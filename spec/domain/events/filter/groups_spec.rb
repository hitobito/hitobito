...
describe Events::Filter::Groups do
  ...
  context 'finds course offerer' do
    ...
    it 'via primary group' do
      expect(subject.send(:course_group_from_primary_layer)).to eq groups(:top_layer)
    end
    ...
    it 'via hierarchy' do
      ...
      expect(other_filter.send(:course_group_from_hierarchy).id).to eq groups(:bottom_layer_one).id
    end
  end
