...
module FormBuilder::GlobalizedInputFieldBuilder
  ...
  def globalized_input_field(attr, rich_text, args = {})
    return (rich_text ? rich_text_area(attr, **args, already_globalized: true) : input_field(attr, **args, already_globalized: true)) unless Globalized.globalize_inputs?

    content_tag(:div, data: {controller: "globalized-fields", "globalized-fields-additional-languages-text-value": I18n.t("global.form.additional_languages").to_s}) do
    ...
    content_tag(:div, class: "d-flex") do
      input_for_locale(attr, I18n.locale, rich_text, **args, value: @object.send(:"#{attr}_#{I18n.locale}")) do
        action_button(nil, nil, "language",
          {"data-action": "globalized-fields#toggleFields", type: "button", in_button_group: true, "data-bs-toggle": "tooltip", "data-bs-title": I18n.t("global.form.input_translation_button")})
      end
    end
  end
    ...
    content_tag(:div, {class: "hidden", "data-globalized-fields-target": "toggle"}) do
      other_locale_inputs = Settings.application.languages.keys.excluding(I18n.locale).map do |locale|
        input_for_locale("#{attr}_#{locale}", locale, rich_text, **args, data: {
          ...
          action: "input->globalized-fields#updateGlobalizedFieldsDisplay trix-change->globalized-fields#updateGlobalizedFieldsDisplay"
  ...
  def input_for_locale(attr, locale, rich_text, args = {})
    content_tag(:div, class: "input-group mb-2") do
      locale_input = locale_indicator(locale) +
        (rich_text ? rich_text_area(attr, **args, already_globalized: true, toolbar: "#{attr}_toolbar") : input_field(attr, **args, already_globalized: true))
  ...
  def locale_indicator(locale)
    content_tag(
      ...
      class: "input-group-text d-flex justify-content-center rounded-start", "data-globalized-fields-target": "localeIndicator",
      "data-bs-toggle": "tooltip", "data-bs-title": I18n.t("global.form.locale_indicator")
    )
  end

  def globalized_field?(attr, already_globalized)
    @object.respond_to?(:translated_attribute_names) && !already_globalized && @object.translated_attribute_names.include?(attr)
  end
end
