...
describe Person::Subscriptions do
  ...
  context :from_groups do
    ...
    it 'is empty when tag required' do
      subscription = list.subscriptions.create!(subscriber: bottom_layer_one, role_types: ['Group::BottomLayer::Member'])
      subscription.update!(tag_list: %w(foo bar))
      expect(subject).to be_empty
    end

    it 'is present when tag required and any person matches' do
      subscription = list.subscriptions.create!(subscriber: bottom_layer_one, role_types: ['Group::BottomLayer::Member'])
      subscription.update!(tag_list: %w(foo bar))
