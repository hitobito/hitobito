...
module TarantulaConfig
  ...
  def configure_urls(t, person)
    ...
    t.allow_500_for(/groups\/\d+\/people\/\d+\/log\/$/)
    t.allow_500_for(/groups\/\d+\/people\/\d+\/history\/$/)
    t.allow_404_for(/groups\/\d+\/people\/\d+\/tags\/\d+$/)
    t.allow_500_for(/groups\/\d+\/people\/\d+\/tags\/\d+$/)
