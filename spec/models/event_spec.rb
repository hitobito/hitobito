...
describe Event do
  ...
  describe "#init_questions" do
    ...
    it "does not add default question of another layer" do
      Event::QuestionTemplate.update_all(group_id: groups(:bottom_layer_one).id)
      e = Event.new(groups: [groups(:top_layer)])
      e.init_questions
  ...
  context "dates" do
    ...
    context "#finish_at" do
      ...
      it "returns latest start_at from all event dates when latest start_at is after latest finish_at" do
        event.dates.create!(start_at: Time.zone.local(2001, 3, 1))
