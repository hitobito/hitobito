...
describe Passes::PassUpdater do
  ...
  describe "::recompute_state!" do
    let(:person) { people(:top_leader) }
    let!(:pass) { Fabricate(:pass, person: person, pass_definition: definition) }
    ...
    it "updates pass to eligible state with validity dates" do
      ...
      described_class.recompute_state!(pass)
    ...
    context "when person has no matching roles" do
      ...
      it "updates pass to revoked state" do
        described_class.recompute_state!(pass)
      ...
      it "preserves the existing valid_from" do
        expect { described_class.recompute_state!(pass) }
          .not_to change { pass.reload.valid_from }
      end
    end

    context "when role has no start_on and pass already has a valid_from" do
      let(:existing_date) { 2.years.ago.to_date }

      before do
        person.roles.first.update_columns(start_on: nil, end_on: 1.year.from_now.to_date)
        pass.update_columns(valid_from: existing_date)
      end

      it "preserves the existing valid_from" do
        described_class.recompute_state!(pass)
        expect(pass.reload.valid_from).to eq(existing_date)
      end
    end
  end
end
