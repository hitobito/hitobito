...
describe Wallets::PassSynchronizer do
  ...
  describe "#sync!" do
    ...
    context "with an apple installation" do
      ...
      it "updates state and sends push notification" do
        push_service = instance_double(Wallets::AppleWallet::PushService)
        expect(Wallets::AppleWallet::PushService).to receive(:new)
          .with(installation)
          .and_return(push_service)
        expect(push_service).to receive(:send_update_notification)
