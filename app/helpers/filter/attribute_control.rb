...
class Filter::AttributeControl # rubocop:disable Rails/HelperInstanceVariable
  ...
  def constraint_options # rubocop:todo Metrics/CyclomaticComplexity
    filters = [[t("filters.attributes.equal"), :equal],
      [t("filters.attributes.blank"), :blank]]
    if type == :string || key.blank?
      filters += [[t("filters.attributes.match"), :match],
        [t("filters.attributes.not_match"), :not_match]]
    end
    if type == :integer || key.blank?
      filters += [[t("filters.attributes.smaller"), :smaller],
        [t("filters.attributes.greater"), :greater]]
    end
    if type == :date || key.blank?
      filters += [[t("filters.attributes.before"), :before],
        [t("filters.attributes.after"), :after]]
    end
    filters
  end
