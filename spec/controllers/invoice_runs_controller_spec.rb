...
describe InvoiceRunsController do
  ...
  context "sheet title" do
    ...
    it "renders Rechnungslauf title for single person" do
      get :new,
        params: {
          group_id: group.id,
          invoice_run: {recipient_ids: person.id},
    ...
    it "renders Rechnungslauf title for multiple people" do
      get :new,
        params: {
          group_id: group.id,
    ...
    it "renders Rechnungslauf title for abo" do
      get :new,
        params: {
          group_id: group.id,
  ...
  context "authorized" do
    ...
    it "POST#create an invoice in background" do
      stub_const("InvoiceRunsController::LIMIT_CREATE", 2)
      Subscription.create!(mailing_list: list, subscriber: groups(:top_group), role_types: [Group::TopGroup::Leader])
    ...
    describe "DELETE#destroy" do
      ...
      it "redirects to list and updates total" do
        ...
        other = Invoice.create!(group: group, title: "test", recipient: person, invoice_run: invoice_run)
        other.invoice_items.create!(name: :pens, count: 1, unit_cost: 10)
        invoice_run.update_total
        invoice_run.invoices.each(&:recalculate!)

        expect do
          travel(1.day) {
            delete :destroy, params: {group_id: group.id, invoice_run_id: invoice_run.id, ids: invoice.id}
          }
        end.to change { invoice.reload.updated_at }
          .and change { invoice_run.reload.amount_total }.from(30).to(10)
