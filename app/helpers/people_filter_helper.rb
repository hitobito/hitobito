...
module PeopleFilterHelper
  def people_filter_qualification_configuration(filter)
    filter ? filter[:qualification].args : {}
  end
  ...
  def people_filter_qualification_validity_options(filter)
    list = %w[active reactivateable not_active_but_reactivateable not_active all none only_expired].map do |option|
      [option, t("people_filters.form.filters_qualification_validity.#{option}")]
    end
