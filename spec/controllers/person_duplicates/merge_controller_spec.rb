...
describe PersonDuplicates::MergeController do
  ...
  context "#create" do
    ...
    it "sets flash error-message detailing which invalid data has been merged anyway" do
      sign_in(layer_leader)
      ...
      expect(response).to redirect_to(group_person_path(person_2.primary_group, person_2))
      expect(flash[:notice]).to eq "Die Personen Einträge wurden trotz gewisser ungültiger Daten zusammengeführt."
      expect(flash[:alert]).to match_array([
