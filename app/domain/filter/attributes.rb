...
module Filter::Attributes
  ...
  def persisted_attribute_condition_sql(key, value, constraint)
    ...
    when /blank/
      "COALESCE(TRIM(#{table_name}.#{key}::text), '') #{sql_comparator(constraint)} ?"
    when /include/
      "#{table_name}.#{key} IN (?)"
    else
      "#{table_name}.#{key} #{sql_comparator(constraint)} ?"
    end
  ...
  def match_search_sql(key, value, constraint)
    if value.is_a?(Numeric)
      "CAST(#{table_name}.#{key} AS TEXT) #{sql_comparator(constraint)} ?"
    else
      "#{table_name}.#{key} #{sql_comparator(constraint)} ?"
    end
  end
