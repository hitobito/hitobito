...
describe Person::ImpersonationController do
  ...
  context 'POST' do
    ...
    it 'impersonates user and sets origin_user' do
      post :create, group_id: group.id, person_id: people(:bottom_member).id
    ...
    it 'cannot impersonate user if current_user' do
      post :create, group_id: group.id, person_id: user.id
    ...
    it 'user without permission cannot impersonate user' do
      post :create, group_id: groups(:bottom_layer_one), person_id: people(:bottom_member)
  ...
  context 'DELETE' do
    ...
    it 'returns to origin user and creates log entry' do
      ...
      expect { delete :destroy, group_id: group.id, person_id: people(:bottom_member).id }.
        to change { PaperTrail::Version.count }.by 1
      expect(controller.send(:origin_user)).to be_nil
    end

    it 'redirects back if no origin_user' do
      delete :destroy, group_id: group.id, person_id: people(:bottom_member).id
      is_expected.to redirect_to(request.env['HTTP_REFERER'])
    end
  end
end
