...
module MailingListFilterHelper

  def mailing_list_filter_attributes_for_select
    Person.filter_attrs.transform_values { |v| v[:label] }.invert.sort
  end

  def mailing_list_filter_types_for_data_attribute
    Person.filter_attrs.transform_values { |v| v[:type] }.to_h.to_json
  end

  def mailing_list_filter_attribute_forms(filter)
    return unless filter

    filter.args.each_with_index.map do |(_k, attr), i|
      mailing_list_filter_attribute_form(attr, i)
    end.join.html_safe
  end

  def mailing_list_filter_attribute_form_template
    mailing_list_filter_attribute_form(nil, 0, disabled: :disabled)
  end

  def mailing_list_filter_attribute_form(attr, count, html_options = {}) # rubocop:disable Metrics/AbcSize,Metrics/MethodLength
