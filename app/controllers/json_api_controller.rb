...
class JsonApiController < ActionController::API
  ...
  def ensure_no_id_param
    ...
    if params.dig(:data, :id).present?
      errors.add("data.id", :must_not_be_set, message: I18n.t("api.errors.resources.id_must_not_be_set"))
    end

    if params.dig(:data, :attributes, :id).present?
      errors.add("data.attributes.id", :must_not_be_set, message: I18n.t("api.errors.resources.id_must_not_be_set"))
    end
