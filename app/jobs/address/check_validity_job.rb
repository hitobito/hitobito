...
class Address::CheckValidityJob < RecurringJob
  ...
  def perform
    ...
    invalid_people_names = invalid_people.map(&:full_name).join(', ')
    Settings.addresses.validity_job_notification_emails.each do |mail_address|
      Address::ValidationChecksMailer.validation_checks(mail_address, invalid_people_names).deliver_later
    end
  end
end
