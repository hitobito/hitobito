...
describe Export::Courses::Row do
  ...
  let(:course) { Fabricate.build(:course, state: :application_closed, groups: [groups(:top_group)],
  ...
  context "dates" do
    ...
    let(:date) { Fabricate.build(:event_date, event: course, start_at: start_at, finish_at: finish_at) }
