...
function setupMultiselectTargets(table) {
  ...
  document.querySelectorAll('a[data-checkable]:not(data-method)').forEach(link => link.addEventListener('click', () => {
    link.href = buildLinkWithIds(link.href, table)
  }))

  // swap href form data-checkable links with method
  document.querySelectorAll('form[data-checkable]').forEach(form => form.addEventListener('submit', () => {
