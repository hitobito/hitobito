...
namespace :wagon do
  namespace :migrate do
    ...
    task status: :environment do
      migrations_paths = wagons.each_with_object({}) {|wagon, hash| hash[wagon.wagon_name] = wagon.migrations_paths }
      migrations_paths['core'] = Rails.application.paths['db/migrate'].to_a
      wagon_names_width = migrations_paths.keys.map(&:length).max

      context = ActiveRecord::MigrationContext.new(migrations_paths.values.flatten, ActiveRecord::SchemaMigration)
      ...
      context.migrations_status.each do |status, version, name|
        ...
        puts "#{status.center(wagon_names_width)} [#{wagon_name.center(8)}] #{version.ljust(14)}  #{name}"
      end
    end
  end
end
