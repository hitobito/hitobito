...
| `openid`        | OIDC identity token (userinfo endpoint)   |
| `api`           | All REST APIs                             |
...
When using the `openid` scope, the flow initially works exactly the same as for OAuth 2.0. The response to the /oauth/token endpoint will contain an additional `id_token` field:
```json
{
  "access_token": "A5t3yMUvKCZLxasqJbQBhu0bRKMvWdwcBSw2DJbUiaQ",
  "token_type": "Bearer",
