    ...
    context :filter do
      ...
      context :active do
        ...
        context "bottom layer" do
          ...
          it "finds role started within range" do
            role.update!(start_on: today)
            f = filter(start_at: today, finish_at: today)
            expect(f.entries).to have(1).item
            expect(f.all_count).to eq(1)
          end

          it "finds past roles only if person is still accessible" do
            role.update!(start_on: 12.months.ago, end_on: 6.month.ago) # no other role in layer, not accessible
            person_with_other_role = Fabricate(:person)
            person_with_other_role.roles.create!(type: role_type.sti_name, group: group, start_on: 2.years.ago, end_on: 9.months.ago)
            person_with_other_role.roles.create!(type: Group::BottomLayer::Leader.sti_name, group: group)
            f = filter(start_at: 10.months.ago, finish_at: 8.months.ago)
            expect(f.entries).to match_array([person_with_other_role])
            expect(f.all_count).to eq(2)
          end
        end
      end
