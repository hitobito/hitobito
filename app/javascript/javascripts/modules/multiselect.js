...
function setupMultiselect() {
	for (const table of document.querySelectorAll("table[data-checkable=true]")) {
...
function setupMultiselectTargets(table) {
	...
	for (const link of document.querySelectorAll(
		"a[data-checkable]:not(data-method)",
	)) {
	...
	for (const form of document.querySelectorAll("form[data-checkable]")) {
...
function buildLinkWithIds(templateHref, table) {
	...
	return (
		templateHref +
		separator +
		(match ? `ids=${[match[1]]}&singular=true` : `ids=${ids}`)
	);
}
