...
Applications must be registered with a URL and allowed scopes (see below). The external application is identified by a UId (unique identifier) and an application secret, both of which are generated by hitobito.
...
An example of the full URL, where your application should redirect the user for asking permission:
```
https://demo.hitobito.ch/oauth/authorize?response_type=code&client_id=df57d88580cd81d2f7ca9974ff5a45dece279a36d4f55fe741502dd3ebb60ba8&redirect_uri=https://my-application.ch/callback&scope=email%20name 
```

If the user accepts the request, the user's browser will be redirected to the specified location, with the authorization code in a query parameter. The authorization code will be available in the code URL parameter. Make sure this is a valid URL in your application. In our example this could be `https://my-application.ch/callback?code=2dd87982736ffbd2b68820a2dbcc1f3bd59c5653c5d166339ce68c320f8d83c5`
...
curl -X POST \
     ...
     -d "redirect_uri=https://my-application.ch/callback" \
...
An example output could be (formatted here for readability):
```json
{
  "id": 34,
