...
describe Event::Participation do
  ...
  context 'confirmation notification' do
    let(:pending_dj_handlers) { Delayed::Job.all.pluck(:handler) }
    let(:pending_participation) do
      participation = course.participations.create!(person: people(:top_leader),
                                                    active: false)
      participation.roles.create!(type: Event::Role::Participant)
      participation
    end
    let(:active_participation) do
      participation = course.participations.create!(person: people(:top_leader),
                                                    active: true)
    ...
    it 'sends confirmation if participation changes from pending to active' do
      expect do
        pending_participation.update!(active: true)
      end.to change { Delayed::Job.count }.by(1)

      expect(pending_dj_handlers.last).to match(/Event::ParticipationConfirmationJob/)
    end

    it 'does not send confirmation if participation not role participant' do
      active_participation.roles.create!(type: Event::Role::Leader)
      expect do
        active_participation.reload.update!(active: true)
      end.to change { Delayed::Job.count }.by(0)
    end

    it 'sends confirmation if participation changes from active to pending' do
      expect do
        # active_participation.update!(active: false)
        active_participation.active = false
        active_participation.save!
      end.to change { Delayed::Job.count }.by(1)

      expect(pending_dj_handlers.last).to match(/Event::ParticipationConfirmationJob/)
    end

    it 'does not send confirmation if active state has not changed' do
      expect do
        active_participation.update!(additional_information: 'Lorem ipsumsum')
      end.to change { Delayed::Job.count }.by(0)
    end

    it 'does not send confirmation when creating new participation' do
      expect do
