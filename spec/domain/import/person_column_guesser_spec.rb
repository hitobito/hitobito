...
describe Import::PersonColumnGuesser do
  ...
  context "maps default values for header" do
    its(["Geschlecht"]) { is_expected.to eq field_for(:gender) }
    its(["vorname"]) { is_expected.to eq field_for(:first_name) }
    its(["skype"]) { is_expected.to eq field_for(:social_account_skype) }

    context "handles noexisting headers" do
      let(:headers) { %w[Geburtsdatum Email] }

      its(["Geburtsdatum"]) { is_expected.to eq nil_key }
      its(["Email"]) { is_expected.to eq nil_key }
    end

    context "params override mapping" do
      let(:params) { {"Name" => "first_name"} }

      its(["Name"]) { is_expected.to eq field_for(:first_name) }
    end
  end
  ...
  context "matching" do
    ...
    context "uses first exact matched value" do
      ...
      its(["Name"]) { is_expected.to eq field_for(:name) }
    end

    context "falls back first partial matched value if no exact match found" do
      let(:headers) { %w[ame] }

      its(["ame"]) { is_expected.to eq field_for(:other_name) }
    end
  end

  def field_for(key)
    field = Import::Person.fields.find { |f| f[:key] == key.to_s }
    fail "no Person field found for #{key}" unless field
    field
  end
end
