...
describe "roles#create", type: :request do
  ...
  before do
    ...
    service_token.update!(register_people: true, permission: :layer_and_below_full, layer_group_id: group.layer_group_id)
  end
    ...
    describe "with already used email" do
      ...
      it "raises validation error" do
        expect {
          ...
          expect(errors[0].message).to include("ist bereits vergeben. Diese Adresse muss für alle Personen eindeutig sein,")
        }.not_to change { [Role.count, Person.count] }
      end
    end
