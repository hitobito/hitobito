...
describe WalletSyncJob do
  ...
  it "continues processing when one installation fails" do
    membership2 = Fabricate(:pass_membership,
      person: Fabricate(:person),
      pass_definition: definition,
      state: :eligible,
      valid_from: 1.month.ago.to_date)
    good_installation = Fabricate(:wallets_pass_installation,
      pass_membership: membership2,
      wallet_type: :apple,
      state: :pending_sync)

    # Make the first installation fail by making its membership invalid
