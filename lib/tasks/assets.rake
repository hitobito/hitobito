...
namespace :assets do
  ...
  task render_scss_entries: :environment do
    ...
    # Prune entries stale under this signature (e.g. a wagon dropping a
    # pack) - can't happen *across* signatures, each has its own directory.
  ...
  task wagon_scss_load_paths: :environment do
    ...
    # --load-path root - passing each wagon's root as an extra one (in
    # build_css.js) fixes that without changing import resolution itself.
