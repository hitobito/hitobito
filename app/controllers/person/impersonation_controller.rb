...
class Person::ImpersonationController < ApplicationController
  ...
  def start_impersonation(person)
    ...
    PaperTrail::Version.create(main: person, item: person, whodunnit: taker, event: :start_impersonation)

    if person.password? && person.email? && Settings.start_impersonation.notify
